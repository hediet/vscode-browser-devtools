...
export class DebugSessionService extends Disposable {
    ...
    getById(debugSessionId: string): DebugSessionProxy | undefined {
        for (const s of this._debugSessions.values()) {
            if (s.session.id === debugSessionId) {
                return s;
            }
        }
        return undefined;
...
export class DebugSessionProxy {
    ...
    public readonly id = this.session.id;
