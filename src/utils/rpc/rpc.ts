...
export interface IMessageSender extends IDisposable {
...
export type RpcRequestResult = { type: 'ok', value: unknown } | { type: 'error', value: unknown };
...
export type MakeSideAsync<T extends RpcSide> = {
    notifications: T['notifications'];
    requests: { [K in keyof T['requests']]: MakeAsyncIfNot<T['requests'][K]> }
};

export class SimpleTypedRpcConnection<T extends RpcSide> implements IDisposable {
    public static createHost<T extends RpcApi>(channel: IChannel, handler: T['host']): SimpleTypedRpcConnection<MakeSideAsync<T['client']>> {
        return new SimpleTypedRpcConnection(channel, handler);
    }

    public static createClient<T extends RpcApi>(channel: IChannel, handler: T['client']): SimpleTypedRpcConnection<MakeSideAsync<T['host']>> {
        return new SimpleTypedRpcConnection(channel, handler);
    }
    ...
    private constructor(
        ...
        private readonly _handler: RpcSide,
    ) {
