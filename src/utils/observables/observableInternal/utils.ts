...
import { BugIndicatingError, DisposableStore, EqualityComparer, Event, IDisposable, strictEquals, toDisposable } from './commonFacade/deps';
...
class ArrayMap<TIn, TOut, TKey> implements IDisposable {
	...
	public getMap(): Map<TKey, TOut> {
		const result = new Map<TKey, TOut>();
		for (const [key, value] of this._cache) {
			result.set(key, value.out);
		}
		return result;
	}
}
